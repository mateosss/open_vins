...
struct TimeStats {
  ...
  public:
	void addTime(const std::string_view name, int64_t ts = INT64_MIN) {
		if (!enabled_exts.has[VIT_TRACKER_EXTENSION_POSE_TIMING]) { return; }
		...
		if (ts == INT64_MIN) { ts = std::chrono::steady_clock::now().time_since_epoch().count(); }

		timings.push_back(ts);
	}

	void addFeature(size_t cam, const vit::PoseFeature &feature) {
		if (!enabled_exts.has[VIT_TRACKER_EXTENSION_POSE_FEATURES]) { return; }
